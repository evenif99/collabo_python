...
print('search 함수는 임의의 위치에서 조건에 만족할 때 출력해 줍니다.')
...
print(f'조건을 충족하는 문자열 : {result.group()}')
print(f'문자열 시작 인덱스 : {start_idx}') # {result.start()}
print(f'문자열 끝 인덱스 : {end_idx}') # {result.end()}
...
addressList = ["('강원원주시웅비2길8');",
          ...
          "('강원강릉시강변로410번길36');"]
print('방법 01')
regEx = '\d\S*' # 숫자로 시작 후 이후에 white character 문자가 아닌 것들
pattern = re.compile(regEx)
...
print('방법 02')
regEx = "\d\S*\'" # 숫자로 시작 후 이후에 외따옴표 까지
pattern = re.compile(regEx)
for address in addressList:
    mymatch = pattern.search(address)
